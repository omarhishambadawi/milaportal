import { parseCoordinatePair, latLngParam } from "./coordinates";
...
/**
 * Hosts whose links are Google Maps links.
 *
 * `maps.app.goo.gl` and `goo.gl` are the shorteners the Google Maps share sheet
 * produces on a phone, which is how most links reach an agent.
 */
const MAPS_HOSTS = /(^|\.)(google\.[a-z.]+|goo\.gl)$/i;

/** Shortened links carry no coordinates at all — only a redirect to one. */
const SHORTENER_HOSTS = /(^|\.)goo\.gl$/i;

/**
 * The place pin, as Google encodes it in the `/data=` blob: `!3d<lat>!4d<lng>`.
 *
 * Preferred over every other form because it is the *place*, whereas `@` below
 * is only where the camera happened to sit. Sharing a pharmacy while scrolled
 * one street away gives two different answers, and this is the one the person
 * meant.
 */
const PLACE_PIN = /!3d(-?\d+(?:\.\d+)?)!4d(-?\d+(?:\.\d+)?)/;

/** The camera: `/@<lat>,<lng>,<zoom>z`. A fallback, not a pin. */
const CAMERA = /@(-?\d+(?:\.\d+)?),(-?\d+(?:\.\d+)?)/;

/** A bare "lat,lng" — what a coordinate-carrying query parameter holds. */
const PAIR = /^\s*(-?\d+(?:\.\d+)?)\s*,\s*(-?\d+(?:\.\d+)?)\s*$/;

/**
 * Query parameters that may carry a coordinate pair.
 *
 * `q` and `query` usually hold a *place name* instead, so a match here is only
 * accepted when it actually looks like a pair — see {@link PAIR}.
 */
 ...
 *
 * Pure and keyless: no SDK, no geocoding call, no credential. An agent pasting
 * a link the customer sent over WhatsApp is the common case, and it should not
 * cost a Places lookup to read the coordinates that are already sitting in the
 * URL.
 *
 * The forms are tried in order of how well each one means "the place the person
 * intended" — the `/data=` pin first, then a coordinate-bearing query
 * parameter, then the map camera. Anything else, including a link naming a
 * place by name only, yields no point rather than a guess.
 *
 * Range checking is delegated to `parseCoordinatePair`, so a pasted link is
 * held to exactly the same bounds as a typed coordinate or an imported
 * spreadsheet cell.
 */
