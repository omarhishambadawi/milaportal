/**
 * Analytics Center — Owner/Administrator only.
 *
 * The validation hub: the single place to answer "why does this KPI not match
 * the PBX report?" without reading thousands of CDR rows by hand.
 *
 * Everything here is production-safe. It returns aggregates and per-call
 * diagnostics only — call ids, extensions, agent names, outcomes and durations.
 * Never credentials, tokens, phone numbers, DIDs, recording paths or raw
 * response bodies. Raw-body endpoint probes live in Diagnostics, which is a
 * separate page precisely because it is a different kind of tool.
 *
...
import { yeastarKpiValidation } from "@/lib/yeastar.functions";
...
export const Route = createFileRoute("/_app/calls/analytics")({
  component: YeastarDiagnostics,
  head: () => ({ meta: [{ title: "Analytics Center — MilaServ Portal" }] }),
});
