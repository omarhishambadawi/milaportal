...
export async function exportDashboard(
  ...
  const teamLbl =
    ctx.teamLabel === "customer_care"
      ? "Customer Care"
      : ctx.teamLabel === "telesales"
        ? "Telesales"
        : "All Teams";
  const summary = [
  ...
  sheet(
    ...
    (data.byDay ?? []).map((d: any) => ({
      Date: d.date,
      "All Sales": d.total,
      "Completed Sales": d.completed,
    })),
  );
  sheet(
    "By Status",
    (data.byStatus ?? []).map((s: any) => ({ Status: s.name, Count: s.value })),
  );
  sheet(
    "By Team",
    (data.byTeam ?? []).map((t: any) => ({
      Team: t.name,
      Orders: t.count,
      "Completed Sales": t.sales,
      "Completion Rate %": Number(t.rate.toFixed(1)),
    })),
  );
  sheet(
    "By Agent",
    (data.byAgent ?? []).map((a: any) => ({
      Agent: a.name,
      Orders: a.count,
      "Completed Sales": a.sales,
      "Completion Rate %": Number(a.rate.toFixed(1)),
    })),
  );
  sheet(
    "By Branch",
    (data.byBranch ?? []).map((b: any) => ({
      Branch: b.name,
      Orders: b.count,
      "Completed Sales": b.sales,
      "Completion Rate %": Number(b.rate.toFixed(1)),
    })),
  );
  sheet(
    "By City",
    (data.byCity ?? []).map((c: any) => ({
      City: c.name,
      Orders: c.count,
      "Completed Sales": c.sales,
      "Completion Rate %": Number(c.rate.toFixed(1)),
    })),
  );
  sheet(
    "Delivery Methods",
    (data.byDelivery ?? []).map((d: any) => ({
      Method: d.name,
      Orders: d.count,
      "Completed Sales": d.sales,
      "Completion Rate %": Number(d.rate.toFixed(1)),
    })),
  );
  sheet(
    "CC Verification by Agent",
    (data.verifAgentRows ?? []).map((r: any) => ({
      Agent: r.name,
      "Total Orders": r.total,
      Verified: r.verified,
      "Non-verified": r.nonVerified,
      "Rate %": Number(r.rate.toFixed(1)),
      "Verified Value": r.verifiedValue,
    })),
  );
  sheet(
    "Complaints by Branch",
    (data.cmpBranchRows ?? []).map((r: any) => ({
      Branch: r.name,
      Total: r.total,
      Resolved: r.resolved,
      Open: r.open,
      "Resolution Rate %": Number(r.rate.toFixed(1)),
    })),
  );
  sheet(
    "Complaints by City",
    (data.cmpCityRows ?? []).map((r: any) => ({
      City: r.name,
      Total: r.total,
      "Resolution Rate %": Number(r.rate.toFixed(1)),
    })),
