/**
 * The Complaints form, shared by `/complaints/$id` and `/complaints/new`.
 *
 * Same reason as `features/orders/components/order-form.tsx`: exported from a
 * route file, it sat in the route shell that `routeTree.gen.ts` imports eagerly,
 * so it and its dependencies (cmdk, zod) shipped in the entry bundle of every
 * page. Moved verbatim.
 */
...
export function ComplaintForm({ mode }: { mode: "create" | "edit" }) {
  ...
  const { data: branches } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("branches").select("branch_no,city").order("branch_no")).data ?? [],
  });
  ...
  if (mode === "create" && !canCreate) {
    return (
      <div className="text-center py-16">
        <ShieldAlert className="mx-auto h-10 w-10 text-destructive" />
        <p className="mt-2 text-sm text-muted-foreground">You don't have permission.</p>
      </div>
    );
  }
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!user) return;
    if (!canEditThis) {
      toast.error("You don't have permission to modify this complaint");
      return;
    }
    setBusy(true);
    try {
  ...
  const del = async () => {
    if (!id) return;
    if (!canDelete) {
      toast.error("You don't have permission to delete complaints");
      return;
    }
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            {mode === "create"
              ? "New complaint"
              : `${readOnly ? "View" : "Edit"} complaint ${existing?.display_no ?? ""}`}
          </CardTitle>
        </CardHeader>
        <CardContent>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-3 h-10 px-3 border rounded-md bg-background">
                    <Checkbox
                      ...
                      disabled={readOnly}
                      onCheckedChange={(v) =>
                        setForm({ ...form, status: v ? "Resolved" : "In Progress" })
                      }
                    />
            ...
            <div className="md:col-span-2 flex justify-end gap-2">
              ...
              {!readOnly && (
                <Button type="submit" disabled={busy}>
                  {busy ? "Saving…" : mode === "create" ? "Save complaint" : "Update complaint"}
                </Button>
              )}
            </div>
          </form>
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [{ data: events }, { data: profiles }] = await Promise.all([
        supabase
          .from("complaint_activity" as any)
          .select("*")
