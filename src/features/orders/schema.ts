...
/**
 * The order as the form holds it, before any AlShrouq rule is applied.
 *
 * Split out because one kind of order must be saved *without* those rules — see
 * `historicalOrderFormSchema` below. Separating the object from the refinement
 * is what lets both exist without either restating the fields.
 */
const orderFormObject = z.object({
...
export const orderFormSchema = orderFormObject
  /**
...
/**
 * The same order, for one that predates the AlShrouq integration.
 *
 * Identical in every respect except that the AlShrouq rules above do not run.
 * An order raised before the integration existed has no location and no payment
 * method and never will — it was delivered by a person on a phone — so requiring
 * them would make the row uneditable: an agent correcting a typo in the notes of
 * a five-month-old order would be told to supply coordinates for a delivery that
 * already happened.
 *
 * Which schema applies is decided from the **stored** row, by
 * `isHistoricalAlShrouqOrder`, never from what is on screen. That is what stops
 * this being a route to saving a *new* AlShrouq order with its delivery missing.
 * The server makes the same determination independently before it will contact
 * the courier, so this choice governs what the form asks for, not whether a
 * duplicate delivery is possible.
 */
export const historicalOrderFormSchema = orderFormObject;
