...
import { navKey, type NavItemData } from "./app-sidebar";
...
export function NavFlyout({
  ...
  const childLink = (c: NavItemData, dense: boolean) => {
    const active = activePath === navKey(c);
    return (
      <Link
        key={navKey(c)}
        to={c.to}
  ...
  /**
   * A child, with whatever chrome precedes it.
   *
   * Two kinds: a plain rule (`separatorBefore`) and a labelled group heading
   * (`groupLabel`). The heading is what lets the CRM say that Cash is a pair of
   * destinations and Wasfaty is one, without a second level of flyout.
   */
  const childBlock = (c: NavItemData, dense: boolean) => (
    <div key={navKey(c)}>
      {c.separatorBefore && (
        <div className={cn("my-1.5 border-t border-border/60", dense && "ml-2.5")} />
      )}
      {c.groupLabel && (
        <p className="px-2.5 pb-0.5 pt-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/70">
          {c.groupLabel}
        </p>
      )}
      {childLink(c, dense)}
    ...
    >
      ...
      {variant === "rail" &&
        ...
        createPortal(
          ...
          >
            {children.map((c) => childBlock(c, false))}
          </div>,
          document.body,
        )}
        ...
        >
          <div className="overflow-hidden">
            <div className="mt-0.5 space-y-0.5 pl-3">
              {children.map((c) => childBlock(c, true))}
            </div>
          </div>
        </div>
      )}
