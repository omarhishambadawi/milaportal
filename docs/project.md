...
- **PBX data is never persisted.** Only an OAuth token cache
  (`yeastar_token_cache`) and fetch-progress rows (`cdr_progress`) touch the
  database; CDR itself lives in per-isolate memory caches only.
...
PostgreSQL on Supabase, `public` schema, PostGIS 3.3.x enabled. 86 migrations in
`supabase/migrations/`.
...
### `cdr_progress`, `yeastar_token_cache`

`service_role` only; no `authenticated` grants.
...
**PBX data is never persisted.** Only the token cache and progress rows touch the
database.
...
2. **Tables missing from generated types.** `admin_activity`, `branch_imports`
   and `branches_nearby` are reached through `as any` casts because the generated
   `types.ts` is regenerated on its own schedule. `geo.functions.ts` confines its
   cast to a single adapter.
...
16. **Add rendering tests.** The suite is 1,040 tests across 31 files, all of pure
...
Facts in this document were checked against the tree at commit `166e86d` on
`main`.
...
Test suite as of writing: **1,040 tests across 31 files, all passing.**
...
`cdr-window`, `cdr-paging`, `cdr-by-number`, `call-report`, `call-report-ttl`,
`lookup-match`, `unanswered`, `yeastar-parity`), the branches feature
